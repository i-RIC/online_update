...
def evatrip_init(cgnsName):
    ...
    read_cgnsName = iric.cg_iRIC_Read_String('inputfile')
    logging.debug('Opening {0}'.format(read_cgnsName))

    common.read_fid = iric.cg_open(read_cgnsName, iric.CG_MODE_READ)
    iric.cg_iRIC_InitRead(common.read_fid)
...
def evatrip_process_steps():
    ...
    for i in range(result_steps):
        ...
        t = iric.cg_iRIC_Read_Sol_Time_Mul(common.read_fid, common.step)
        iric.cg_iRIC_Write_Sol_Time_Mul(common.write_fid, t)
        common.time = t
...
def _output_final_time():
    count = iric.cg_iRIC_Read_Sol_Count_Mul(common.read_fid)
    lasttime = iric.cg_iRIC_Read_Sol_Time_Mul(common.read_fid, count)
    t = 1
    while t <= lasttime:
        t *= 10
...
def _logging_init():
    log_level = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'log_level')
    level = logging.INFO
    if log_level == 1:
        level = logging.DEBUG
...
def _set_result_names():
    common.result_depth_name = iric.cg_iRIC_Read_String_Mul(common.write_fid, 'result_depth')
    logging.debug('depth value read from {0}'.format(common.result_depth_name))
    common.result_water_surface_elevation_name = iric.cg_iRIC_Read_String_Mul(common.write_fid, 'result_wse')
    logging.debug('water surface elevation value read from {0}'.format(common.result_water_surface_elevation_name))
    common.result_velocityX_name = iric.cg_iRIC_Read_String_Mul(common.write_fid, 'result_vx')
    logging.debug('velocity x value read from {0}'.format(common.result_velocityX_name))
    common.result_velocityY_name = iric.cg_iRIC_Read_String_Mul(common.write_fid, 'result_vy')
...
def _load_functions():
    ...
    f_riffle_pool = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'f_riffle_pool')
    f_statistics = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'f_statistics')
    f_classification = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'f_classification')
    f_composition = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'f_composition')
    f_response = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'f_response')
