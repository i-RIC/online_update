...
def init():
    ...
    target = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_target')
    logging.debug('target = {0}'.format(target))
    if target == 3:
        target_other = iric.cg_iRIC_Read_String_Mul(common.write_fid, 'stat_target_other')
        logging.debug('target_other = {0}'.format(target_other))
    omit_dry_area = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_dryarea_enabled')
    logging.debug('omit_dry_area = {0}'.format(omit_dry_area))
    dry_area_threshold = iric.cg_iRIC_Read_Real_Mul(common.write_fid, 'stat_dryarea_threshold')
    logging.debug('dry_area_threshold = {0}'.format(dry_area_threshold))

    group_num = iric.cg_iRIC_Read_Complex_Count_Mul(common.write_fid, "stat_area")
    logging.debug('group_num = {0}'.format(group_num))
    group_val = iric.cg_iRIC_Read_Grid_Complex_Node_Mul(common.write_fid, "stat_area")

    time_whole = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_timeranage_whole')
    logging.debug('time_whole = {0}'.format(time_whole))
    time_start = iric.cg_iRIC_Read_Real_Mul(common.write_fid, 'stat_timerange_start')
    logging.debug('time_start = {0}'.format(time_start))
    time_end = iric.cg_iRIC_Read_Real_Mul(common.write_fid, 'stat_timerange_end')
    ...
    output_max = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_max_enabled')
    logging.debug('output_max = {0}'.format(output_max))
    output_min = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_min_enabled')
    logging.debug('output_min = {0}'.format(output_min))
    output_avg = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_avg_enabled')
    logging.debug('output_avg = {0}'.format(output_avg))
    output_stddev = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_stddev_enabled')
    logging.debug('output_stddev = {0}'.format(output_stddev))
    output_stddev_div_avg = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_stddev_avg_enabled')
    logging.debug('output_stddev_div_avg = {0}'.format(output_stddev_div_avg))

    csv_enable = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'stat_csv_enable')
    logging.debug('csv_enable = {0}'.format(csv_enable))
    csv_name = iric.cg_iRIC_Read_String_Mul(common.write_fid, 'stat_csv_name')
...
def process_step():
    ...
    if output_max:
        iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_MAX, zeros)
        _update_node_max(vals)

    if output_min:
        iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_MIN, zeros)
        _update_node_min(vals)

    if output_avg:
        iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_AVG, zeros)

    if output_stddev:
        iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_STDDEV, zeros)

    if output_stddev_div_avg:
        iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_STDDEV_AVG, zeros)
...
def finalize():
    ...
    if output_max:
        if node_max is None:
            ...
            iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_MAX, node_max)
    ...
    if output_min:
        if node_min is None:
            ...
            iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_MIN, node_min)
    ...
    if output_avg:
        if node_sum is None:
            ...
            iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_AVG, node_avg)
    ...
    if (output_stddev_div_avg or output_stddev_div_avg) and not (node_sum is None):
        ...
        result_steps = iric.cg_iRIC_Read_Sol_Count_Mul(common.read_fid)
        for i in range(result_steps):
            step = i + 1
            t = iric.cg_iRIC_Read_Sol_Time_Mul(common.read_fid, step)
    ...
    if output_stddev:
        if node_stddev is None:
            ...
            iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_STDDEV, node_stddev)
    ...
    if output_stddev_div_avg:
        if node_stddev is None:
            ...
            iric.cg_iRIC_Write_Sol_Real_Mul(common.write_fid, OUTPUT_STDDEV_AVG, node_stddev / node_avg)
...
def _output_area_statistics_header():
    ...
    for i in range(group_num):
        group_name = iric.cg_iRIC_Read_Complex_String_Mul(common.write_fid, "stat_area", i + 1, "_caption")
...
def _get_target_value():
    if target == 0:
        ...
        return iric.cg_iRIC_Read_Sol_Real_Mul(common.read_fid, common.step, target_other)
...
def _get_target_value_at(step):
    ...
    return iric.cg_iRIC_Read_Sol_Real_Mul(common.read_fid, step, target_name)
