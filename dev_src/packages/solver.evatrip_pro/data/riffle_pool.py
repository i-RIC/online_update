...
def init():
    ...
    d1 = iric.cg_iRIC_Read_Real_Mul(common.write_fid,'rp_d1')
    logging.debug('rp_d1 = {0}'.format(d1))
    v1 = iric.cg_iRIC_Read_Real_Mul(common.write_fid,'rp_v1')
    logging.debug('rp_v1 = {0}'.format(v1))
    v2 = iric.cg_iRIC_Read_Real_Mul(common.write_fid,'rp_v2')
    logging.debug('rp_v2 = {0}'.format(v2))

    output_riffle = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'rp_output_riffle')
    logging.debug('rp_output_riffle = {0}'.format(output_riffle))
    output_pool = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'rp_output_pool')
    logging.debug('rp_output_pool = {0}'.format(output_pool))
    output_rapid = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'rp_output_rapid')
    logging.debug('rp_output_rapid = {0}'.format(output_rapid))

    output_fr_classes = iric.cg_iRIC_Read_Integer_Mul(common.write_fid, 'rp_output_fr_classes')
...
def _process_step_manual():
    if output_pool:
        ...
        pool = cond1 * cond2
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_POOL, pool)
    ...
    if output_riffle:
        ...
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_RIFFLE, riffle)

    if output_rapid:
        cond1 = np.where(common.result_depth <= d1, 1, 0)
        cond2 = np.where(common.result_velocity >= v2 , 1, 0)
        rapid = cond1 * cond2
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_RAPID, rapid)
...
def _process_step_ent():
    ...
    if output_fr_classes:
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_FR_CLASSES, classes)

def _finalize_manual():
    if output_pool:
        zeros = np.zeros(common.result_depth.shape, dtype=np.int8)
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_POOL, zeros)

    if output_riffle:
        zeros = np.zeros(common.result_depth.shape, dtype=np.int8)
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_RIFFLE, zeros)

    if output_rapid:
        zeros = np.zeros(common.result_depth.shape, dtype=np.int8)
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_RAPID, zeros)

def _finalize_ent():
    zeros = np.zeros(common.result_depth.shape, dtype=np.int8)

    if output_fr_classes:
        iric.cg_iRIC_Write_Sol_Integer_Mul(common.write_fid, OUTPUT_FR_CLASSES, zeros)
